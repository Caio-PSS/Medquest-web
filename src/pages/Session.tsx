...
const Session = () => {
    ...
    const fetchCategories = async () => {
      try {
        const res = await fetch(
          `${process.env.REACT_APP_API_URL}/api/questions/categories-subtopics`,
          {
            headers: { 
  ...
  const loadQuestions = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setQuestions(data);
      setTimeStarted(Date.now()); // Resetar timer
  ...
  const handleAnswer = async (answer: string, isCorrect: boolean) => {
    ...
    try {
      await fetch(`${process.env.REACT_APP_API_URL}/api/respostas`, {
        method: 'POST',
        headers: {
 ...
 // Novo método para avançar manualmente
            ...
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-6">
                ...
                <span className="text-gray-400">
                  Tempo: {Math.floor((Date.now() - timeStarted) / 1000)}s
                </span>
              </div>
            </div>
          </div>
